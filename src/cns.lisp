;;; CNS Interpreter in Common Lisp - v1.8.0
...
;;;
;;; v1.8.0 (November 4, 2025): CLI Arguments & File Operations
;;; - CLI arguments: ARGS[n], ARG(), HAS_FLAG()
;;; - File operations: FILE EXISTS, LIST FILES, DELETE FILE, RENAME FILE
;;; - Expression parser refactoring: Fixed FILE EXISTS bug
;;; - Architecture improvements: Consistent parser patterns, zero warnings
...
(defun quoted-string-p (str)
  "Check if STR is a quoted string (starts and ends with double quotes)."
  (and (> (length str) 1)
       (char= (char str 0) #\")
...
(defun can-parse-parse-json-p (trimmed)
  "Check if TRIMMED is a PARSE JSON operation."
  (starts-with (string-upcase trimmed) "PARSE JSON "))

(defun try-parse-json (trimmed env)
  "Parse and evaluate PARSE JSON operation.
   Supports:
   - PARSE JSON {expr} GET \"key\"
   - PARSE JSON {expr} GET \"path.to.value[0]\"
   - PARSE JSON {expr} GET \"key\" LENGTH
   - PARSE JSON {expr} LENGTH
   - PARSE JSON {expr}"
  (when (can-parse-parse-json-p trimmed)
...
(defun can-parse-list-at-p (trimmed)
  "Check if TRIMMED is a list AT operation: list_var AT index"
  (search " AT " (string-upcase trimmed)))

(defun try-list-at (trimmed env)
  "Parse and evaluate list AT operation to get item at index (0-based)."
  (when (can-parse-list-at-p trimmed)
...
(defun can-parse-property-access-p (trimmed)
  "Check if TRIMMED contains property access: object.property"
  (position #\. trimmed))

(defun try-property-access (trimmed env)
  "Parse and evaluate property access: object.property (e.g., request.method)"
  (when (can-parse-property-access-p trimmed)
...
;;; ============================================================================
;;; Socket and HTTP Operations
;;; ============================================================================

(defun can-parse-create-socket-p (trimmed)
  "Check if TRIMMED is CREATE socket ON port operation."
  (starts-with (string-upcase trimmed) "CREATE "))

(defun try-create-socket (trimmed env)
  "Parse and evaluate CREATE socket ON port operation."
  (when (can-parse-create-socket-p trimmed)
...
(defun can-parse-accept-connection-p (trimmed)
  "Check if TRIMMED is ACCEPT connection ON socket operation."
  (starts-with (string-upcase trimmed) "ACCEPT "))

(defun try-accept-connection (trimmed env)
  "Parse and evaluate ACCEPT connection ON socket operation."
  (when (can-parse-accept-connection-p trimmed)
    (let* ((parts (split-string trimmed #\Space))
           (on-idx (position "on" parts :test #'string-equal)))
      (when on-idx
...
(defun can-parse-parse-http-request-p (trimmed)
  "Check if TRIMMED is PARSE HTTP REQUEST FROM variable operation."
  (starts-with (string-upcase trimmed) "PARSE HTTP REQUEST "))

(defun try-parse-http-request (trimmed env)
  "Parse and evaluate PARSE HTTP REQUEST FROM variable operation."
  (when (can-parse-parse-http-request-p trimmed)
...
(defun can-parse-find-route-p (trimmed)
  "Check if TRIMMED is FIND ROUTE operation."
  (starts-with (string-upcase trimmed) "FIND ROUTE "))

(defun try-find-route (trimmed env)
  "Parse and evaluate FIND ROUTE operation (simplified for now)."
  (when (can-parse-find-route-p trimmed)
    ;; Simplified route matching - will enhance later
    ;; For now, just return found or not found
    :route-not-found))

;;; ============================================================================
;;; Legacy Multiply Action (side-effect operation)
;;; ============================================================================
...
(defun try-multiply-action (trimmed env)
  "Parse and execute MULTIPLY action (side effect - mutates variable).
   ...
   Example: MULTIPLY result BY 2"
  (when (can-parse-multiply-p trimmed)
    (let ((parts (split-string trimmed #\Space)))
...
(defun parse-cns (code)
  ...
  (if (search "---" code)
      ...
      (mapcar #'parse-single-cns (split-multi-story-code code))
      ;; Single story - parse and return as-is
      (parse-single-cns code)))
...
;; FIND effect helper
(defun can-handle-find-effect-p (trimmed)
  "Check if TRIMMED is a FIND effect."
  (starts-with (string-upcase trimmed) "FIND "))

(defun handle-find-effect (trimmed env verbose)
  "Execute FIND (file search) effect.
  ...
  (when (can-handle-find-effect-p trimmed)
    (let* ((rest (trim (subseq trimmed 5)))
           (rest-upper (string-upcase rest))
...
;; GIT operations effect helper
(defun can-handle-git-effect-p (trimmed)
  "Check if TRIMMED is a GIT effect."
  (starts-with (string-upcase trimmed) "GIT "))

(defun handle-git-effect (trimmed env verbose)
  "Execute GIT operations effect.
   Supports: CLONE, STATUS, CHECKOUT, DIFF, ADD, COMMIT, BRANCH, MERGE, LOG"
  (when (can-handle-git-effect-p trimmed)
       ...
       ((string= subcommand "checkout")
        ...
        ((string= subcommand "diff")
         (let* ((after-diff (trim (subseq rest 4)))
                ...
                (file-spec (when (and (> (length after-diff) 0) (char= (char after-diff 0) #\"))
           ...
           (cond
            ...
            ((string= operation "create")
             (let* ((after-op (trim (subseq after-branch 7)))
                    ...
                    (branch-name (if name-end (subseq after-op 1 name-end) nil))
                   ...
                   (handler-case
                       (let* ((process (sb-ext:run-program "/usr/bin/git"
                         ...
                         (when status-var
                           (setf (gethash status-var env) exit-code))
                         (when verbose
                           (format t "  Effect: GIT BRANCH CREATE ~A (exit: ~A)~%" branch-name exit-code))
                         t)
                     (error (e)
                       (when status-var (setf (gethash status-var env) 1))
                       (when verbose
                   ...
                   (handler-case
                       (let* ((process (sb-ext:run-program "/usr/bin/git"
                         ...
                         (when status-var
                           (setf (gethash status-var env) exit-code))
                         (when verbose
                           (format t "  Effect: GIT BRANCH DELETE ~A (exit: ~A)~%" branch-name exit-code))
                         t)
                     (error (e)
                       (when status-var (setf (gethash status-var env) 1))
                       (when verbose
            ...
            ;; GIT BRANCH (list branches) INTO output WITH EXIT_CODE code
            (t
             (let* ((into-pos (search " INTO " after-branch-upper))
                    (exit-pos (search " WITH EXIT_CODE " after-branch-upper))
                    (output-var (when (and into-pos exit-pos)
                                 (trim (subseq after-branch (+ into-pos 6) exit-pos))))
                    (exit-var (when exit-pos
...
;; CSV WRITE effect helper
(defun can-handle-csv-write-effect-p (trimmed)
  "Check if TRIMMED is a CSV WRITE effect."
  (starts-with (string-upcase trimmed) "CSV WRITE "))

(defun handle-csv-write-effect (trimmed env verbose)
  "Execute CSV WRITE effect.
   Syntax: CSV WRITE data TO \"filepath\" WITH HEADERS headers"
  (when (can-handle-csv-write-effect-p trimmed)
    (let* ((rest (trim (subseq trimmed 10)))
           (to-pos (search " TO " (string-upcase rest)))
...
(defun handle-grep-effect (trimmed env verbose)
  "Execute GREP (content search) effect.
  ...
  (when (can-handle-grep-effect-p trimmed)
    (let* ((rest (trim (subseq trimmed 5)))
           (rest-upper (string-upcase rest))
           ;; Extract the pattern (quoted string)
           (pattern-end (position #\" rest :start 1))
...
;; NOTE: auto-fix-literal-first function removed!
;; No longer needed - operator checks now come BEFORE number literal parsing,
;; so "3 * n" naturally parses as multiplication without needing auto-fix.
          ...
          (let ((trimmed (trim expr)))
           ...
           ;; Comparison: n > 1 (must come AFTER >= check)
            ...
            ((can-parse-comparison-simple-p trimmed #\<)
             ...
             ;; This allows "3 * n" to work while "n % 2 = 0" is handled as comparison
             ;; BUT: Don't match operators inside quoted strings like "YYYY-MM-DD"
             ;; ========================================================================
            ...
            ((can-parse-binary-operator-p trimmed #\/)
             ...
             ;; Only match if the ENTIRE string is a valid number (no operators)
             ((and (> (length trimmed) 0)
                   (or (digit-char-p (char trimmed 0))
                       (and (char= (char trimmed 0) #\-)
                            (> (length trimmed) 1)))
                   ;; Make sure there are no operators in the string
                   (not (search "*" trimmed))
                   (not (search "/" trimmed))
                   (not (search "+" trimmed))
                   (not (search "%" trimmed))
                   ;; Only allow - at the start (negative numbers)
                   (not (position #\- trimmed :start 1))
                   (handler-case
                       ;; Validate that read-from-string consumes the ENTIRE string
                       (multiple-value-bind (value position)
                           (read-from-string trimmed)
                         (and (numberp value) (= position (length trimmed))))
                     (error () nil)))
              ;; Use read-from-string to handle both integers and floats
              (read-from-string trimmed))
           
             ;; NOTE: MIN OF, MAX OF, RANDOM FROM, RANDOM moved earlier (line ~1767)
             ...
             ((can-parse-boolean-not-p trimmed)
              ...
              ((can-parse-parse-json-p trimmed)
               (try-parse-json trimmed env))
             ...
             ;; This prevents "ABS OF -100" from being parsed as subtraction
            
             ;; NOTE: Math functions moved earlier in file (line ~1712):
             ;; - SQRT, POW, ABS, ROUND, FLOOR, CEIL, MIN, MAX, RANDOM
             ;; They must come BEFORE arithmetic operators to avoid parsing conflicts
             ...
             ((can-parse-list-at-p trimmed)
              (try-list-at trimmed env))
             
             ;; Property access: object.property (e.g., request.method)
             ((can-parse-property-access-p trimmed)
              (try-property-access trimmed env))
            
             ;; Socket: Create server socket on port
             ((can-parse-create-socket-p trimmed)
              (try-create-socket trimmed env))
             
             ;; Socket: Accept connection on socket
             ((can-parse-accept-connection-p trimmed)
              (try-accept-connection trimmed env))
             
             ;; HTTP: Parse HTTP request from variable
             ((can-parse-parse-http-request-p trimmed)
              (try-parse-http-request trimmed env))
             
              ;; Socket: Find route
              ((can-parse-find-route-p trimmed)
               (try-find-route trimmed env))
              
              ;; Action: Multiply result by n (legacy side-effect operation)
              ((can-parse-multiply-p trimmed)
               (try-multiply-action trimmed env))
